...
class ResultController extends Controller
{
    ...
    public function behaviors()
    {
         return [
            'access' => [
                ...
                'only' => ['mega645', 'vietlott655','max4d','crawler-ket-qua-xo-so'],
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['mega645', 'vietlott655','max4d','crawler-ket-qua-xo-so'],
                        'roles' => ['?'],
                    ],
                    [
                        'allow' => true,
    ...
    public function getColumnXoSoMienNam($url){
    ...
    public function getColumnXoSoMienNam1($url,$maTinh,$i){
        $document = new Document();
        $document->loadHtmlFile($url);
        $codeMaTinh = $maTinh[4];
        $input = [];
    ...
    public function getColumnXoSoMienTrung($url,$codeMaTinh,$i){
        $document = new Document();
        $document->loadHtmlFile($url);
        $input = [];
        $input[$i][0] = $document->find("td[class='" . $codeMaTinh . "_g0 red']")[0];
    ...
    public function getColumnXoSoMienTrungTheoNgay($url){
        $document = new Document();
        $document->loadHtmlFile($url);
        $input = [];
        $input[$i][0] = $document->find('td')[42];
    ...
    public function getColumnXoSoMienBac($url){
        $document = new Document();
        $document->loadHtmlFile($url);
        $input = [];
    ...
    public function getColumnXoSoMienBacTheoNgay($url){

        $document = new Document();
        $document->loadHtmlFile($url);
        $input = [];
        echo $document->find('td')[38];
        die();
    ...
    public function getLichQuay($weekdays){
        switch ($weekdays) {
            case 'monday':
                $weekdays = 'T.2';
    ...
    public function getLichQuaySoiCau($weekdayName){
        switch($weekdayName) {
            case 'monday':
                $weekdayName = 'T.2';
        ...
        return  ([
            'mt'=>$mt,
            'mn'=>$mn,
			'mientrungName'=> $mientrungName,
			'miennamName'=> $miennamName,
        ]);
    }
    public function gettColumnMega645($url){
        $document = new Document();
        $document->loadHtmlFile($url);
    ...
    public function gettColumnVietlott655($url){
        $document = new Document();
        $document->loadHtmlFile($url);
    ...
    public function getColumnMax4d($url,$inputKi = null){
        $document = new Document();
        $document->loadHtmlFile($url);
        if($inputKi < 259 && $inputKi != null && $inputKi != ''){
            $ngay = $document->find('h5 > b')[1];
            $g1 = $document->find('td[class=color_red nowrap]')[0];
            $g2 = preg_replace('/class="color_red nowrap">/', '', explode(' ', $document->find('td[class=color_red nowrap]')[1]));
            $g21 = $g2[1];
            $g22 = $g2[2];
            $g3 = preg_replace('/class="color_red nowrap">/', '', explode(' ', $document->find('td[class=color_red nowrap]')[2]));
            $g31 = $g3[1];
            $g32 = $g3[2];
            $g33 = $g3[3]; 
            ...
            $ki = $document->find('h5 > b')[0];
            $ngay = $document->find('h5 > b')[1];
            $kyValue = preg_replace('/#/', '', $ki);
        }else{
            $ngay = $document->find('h5 > b')[1];
    ...
    public function actionCrawlerKetQuaXoSo(){
        ...
        if ((strtotime($time) > strtotime("18:00:00") && strtotime($time) < strtotime("19:00:00"))) {
            ...
            $stripTagStr = ResultController::getColumnXoSoMienBac($url);
        ...
        elseif ((strtotime($time) > strtotime("17:00:00") && strtotime($time) < strtotime("18:00:00"))) {
            ...
            $lichQuay = ResultController::getLichQuay($weekdays);
            $i = 0;
            foreach ($lichQuay['mt'] as $maTinh) {
                $url = "http://ketqua.vn/kqxs$maTinh[1]-ket-qua-xo-so-$maTinh[2]";
                
                $codeMaTinh = $maTinh[4];
                
                $stripTagStr = ResultController::getColumnXoSoMienTrung($url,$codeMaTinh,$i);
        ...
        elseif (strtotime($time) > strtotime("16:00:00")) {
            ...
            $lichQuay = ResultController::getLichQuay($weekdays);
            $i = 0;
            foreach ($lichQuay['mn'] as $maTinh) {
                $url = "http://ketqua.vn/kqxs$maTinh[1]-ket-qua-xo-so-$maTinh[2]";

                $stripTagStr = ResultController::getColumnXoSoMienNam1($url,$maTinh,$i);
        ...
        if ($dateToDate != null || $dateToDate != ''){
            ...
            foreach ($listDates as $listDate){
                ...
                if ($listDate < $dateNow){
                    // get data crawler
                    //mien bac
                    $stripTagStr = ResultController::getColumnXoSoMienBacTheoNgay($url);
                 // check trong result co du lieu chua
                    ...
                    $i = 0;
                    foreach ($lichQuay['mt'] as $maTinh) {
                        $url = "http://ketqua.vn/kqxs$maTinh[1]-ket-qua-xo-so-$maTinh[2]-ngay-".date('d-m-Y',strtotime($listDate));
                        
                        // mien trung theo ngay
                        $stripTagStr = ResultController::getColumnXoSoMienTrungTheoNgay($url);
                    ...
                    foreach ($lichQuay['mn'] as $maTinh) {
                        ...
                        $stripTagStr = ResultController::getColumnXoSoMienNam($url);
        ...
        if ($inputKi != '' && $inputKi != null){
            ...
            if( (int)$lengthInput == 5){
                ...
                if( (int)strip_tags($kyValueHienTai) < (int)$inputKi ){
                    ...
                    $dataProvider->orderBy(['ki'=>SORT_DESC]);
                    Yii::$app->session->setFlash('success', Yii::t('app','Kì tìm kiếm không hợp lệ vui lòng kiểm tra lại'));
                }else{
                   
                    $dataColumn = ResultController::gettColumnVietlott655($url);

                    $check = Power655::find()->andWhere(['ki' => $inputKi])->count();
                    if ($check > 0) {
                        $model = Power655::find()->where(['ki' => $inputKi])->one();
                        $model->so1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num1'])));
                        $model->so2 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num2'])));
                        $model->so3 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num3'])));
                        $model->so4 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num4'])));
                        $model->so5 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num5'])));
                        $model->so6 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num6'])));
                        $model->so7 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['num7'])));
                        $model->g1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['giaiNhat'])));
                        $model->g2 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['giaiNhi'])));
                        $model->g3 = strip_tags(trim(preg_replace('/[\t\s\.\,]/', '', $dataColumn['giaiBa'])));
                        $model->jackport1 = strip_tags(trim(preg_replace('/[\t\s\.\,]/', '', $dataColumn['jackpot1'])));
                        $model->jackport2 = strip_tags(trim(preg_replace('/[\t\s\.\,]/', '', $dataColumn['jackpot2'])));
                        $model->j1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['j1'])));
                        $model->j2 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['j2'])));
                        $model->ki = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['kyValue'])));
                        $model->ngay = date('Y-m-d',strtotime(strip_tags(trim(preg_replace('/b/', '', str_replace('/', '-', $dataColumn['ngay']))))));
                        if ($model->save()) {
                            $productjson = Json::encode($model);
            ...
            $searchModel = new Power655Query();
            $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
            $dataProvider->query->orderBy(['ki' => SORT_DESC]);
        }
        ...
        if ($inputKi != '' && $inputKi != null){
            ...
            if( (int)$lengthInput == 5){
                ...
                if( (int)strip_tags($kyValueHienTai) < (int)$inputKi ){
                    ...
                    $dataColumn = ResultController::gettColumnMega645($url);
                    ...
                    if ($check > 0) {
                        ...
                        $model->g2 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['giaiNhi'])));
                        $model->g3 = strip_tags(trim(preg_replace('/[\t\s\.\,]/', '', $dataColumn['giaiBa'])));
                        $model->jackport = strip_tags(trim(preg_replace('/[\t\s\.\,]/', '', $dataColumn['jackpot1'])));
                        $model->g0 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['j1'])));
                        $model->ki = $inputKi;
                        $model->ngay = date('Y-m-d',strtotime(strip_tags(trim(preg_replace('/b/', '', str_replace('/', '-', $dataColumn['ngay']))))));
        ...
        if ($inputKi != '' && $inputKi != null){
            ...
            if( (int)$lengthInput == 5){
                ...
                if( (int)strip_tags($kyValueHienTai) < (int)$inputKi ){
                    ...
                    $url = "https://vietlott.vn/vi/trung-thuong/ket-qua-trung-thuong/max-4d?id=".$inputKi."&nocatche=1";
                    ...
                    if ($inputKi < 259){
                        ...
                        $dataColumn = ResultController::getColumnMax4d($url,$inputKi);
                        ...
                        if ($check > 0) {
                            ...
                            $model->ki = $inputKi;
                            $model->ngay = date('Y-m-d',strtotime(strip_tags(trim(preg_replace('/b/', '', str_replace('/', '-', $dataColumn['ngay']))))));
                            $model->g1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['g1'])));
                            $model->g21 = strip_tags(trim(preg_replace('/nowrap">/', '', $dataColumn['g21'])));
                            $model->g22 = strip_tags(trim(preg_replace('/nowrap">/', '', $dataColumn['g22'])));
                            $model->g31 = strip_tags(trim(preg_replace('/nowrap">/', '', $dataColumn['g31'])));
                            $model->g32 = strip_tags(trim(preg_replace('/nowrap">/', '', $dataColumn['g32'])));
                            $model->g33 = strip_tags(trim(preg_replace('/nowrap">/', '', $dataColumn['g33'])));
                            $model->g4 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['g4'])));
                            $model->g5 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['g5'])));
                            $model->t1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t2'])));
                            $model->t2 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t3'])));
                            $model->t3 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t4'])));
                            $model->t4 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t5'])));
                            $model->t5 = (int)strip_tags(trim(preg_replace('/[,]/', '', $dataColumn['t6'])));
                            $model->th41 = 0;
                            $model->th42 = 0;
                            $model->th43 = 0;
                            $model->th61 = 0;
                        ...
                        if ($check > 0) {
                            ...
                            $model->ngay = date('Y-m-d',strtotime(strip_tags(trim(preg_replace('/b/', '', str_replace('/', '-', $dataColumn['ngay']))))));
                            $model->g1 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g1'])));
                            $model->g21 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g21'])));
                            $model->g22 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g22'])));
                            $model->g31 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g31'])));
                            $model->g32 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g32'])));
                            $model->g33 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g33'])));
                            $model->g4 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g4'])));
                            $model->g5 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['g5'])));
                            $model->t1 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t2'])));
                            $model->t2 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['t3'])));
                            $model->t3 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['t4'])));
                            $model->t4 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['t5'])));
                            $model->t5 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t6'])));
                            $model->th41 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t22'])));
                            $model->th42 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['t32'])));
                            $model->th43 = strip_tags(trim(preg_replace('/[\t\s]/', '',  $dataColumn['t42'])));
                            $model->th61 = strip_tags(trim(preg_replace('/[\t\s]/', '',   $dataColumn['t23'])));
                            $model->th62 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t33'])));
                            $model->th63 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t43'])));
                            $model->th121 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t24'])));
                            $model->th122 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t34'])));
                            $model->th123 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t44'])));
                            $model->th241 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t25'])));
                            $model->th242 = strip_tags(trim(preg_replace('/[\t\s]/', '',$dataColumn['t35'])));
                            $model->th243 = strip_tags(trim(preg_replace('/[\t\s]/', '', $dataColumn['t45'])));
                            if ($model->save()) {
                                $productjson = Json::encode($model);
                                $jsonfile = Yii::getAlias('@webroot/json/max4d.json');
                                $fp = fopen($jsonfile, 'w+');
    ...
    public function  actionSoiCauMienBac(){
        ...
        if (Yii::$app->request->isPost) {
            // data post
            $a2 = Yii::$app->request->post('a2');
            $a3 = Yii::$app->request->post('a3');
            $a4 = Yii::$app->request->post('a4');
			$a5 = Yii::$app->request->post('a5');
			$a6 = Yii::$app->request->post('a6');
			
            $b2 = Yii::$app->request->post('b2');
            $b3 = Yii::$app->request->post('b3');
            $b4 = Yii::$app->request->post('b4');
            $b5 = Yii::$app->request->post('b5');
			$b6 = Yii::$app->request->post('b6');
			$b7 = Yii::$app->request->post('b7');
            
			$d1 = Yii::$app->request->post('d1');
            $d2 = Yii::$app->request->post('d2');
            $d3 = Yii::$app->request->post('d3');
            $d4 = Yii::$app->request->post('d4');
			
			$e1 = Yii::$app->request->post('e1');
            $e2 = Yii::$app->request->post('e2');
            $e3 = Yii::$app->request->post('e3');
            $e4 = Yii::$app->request->post('e4');
			
			$f1 = Yii::$app->request->post('f1');
            $f2 = Yii::$app->request->post('f2');
            $f3 = Yii::$app->request->post('f3');
            $f4 = Yii::$app->request->post('f4');
			
			$g1 = Yii::$app->request->post('g1');
            $g2 = Yii::$app->request->post('g2');
            $g3 = Yii::$app->request->post('g3');
            $g4 = Yii::$app->request->post('g4');
            // tạo ra 1 mảng dữ liệu       

            $content["date"] = $date;
			$content["data"] = array($a2,$a3,$a4,$a5,$a6,$b2,$b3,$b4,$b5,$b6,$b7);
			$content["soicau"][0]=array($d1,$d2,$d3,$d4);
			$content["soicau"][1]=array($e1,$e2,$e3,$e4);
			$content["soicau"][2]=array($f1,$f2,$f3,$f4);
			$content["soicau"][3]=array($g1,$g2,$g3,$g4);

            // check tồn tại theo ngày
            $check = SoiCau::find()->where(['date'=>$date])->andWhere(['province'=>136])->count();
            if($check > 0){
                $model = SoiCau::find()->where(['date'=>$date])->andWhere(['province'=>136])->one();
                $model->province = 136;
                $model->date = $date;
                $model->data = Json::encode($content,true);
                $model->create_date = date('Y-m-d H:i:s');
				
                if($model->save()){
                    $productjson = Json::encode($content,true);
		...
		$path = Yii::getAlias('@webroot/json/soi-cau-mien-bac.json');
    ...
    public function  actionSoiCauMienTrung(){
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $time = date('H:i:s');
        $date = date('Y-m-d');
        // echo $time;
        if(strtotime($time) >strtotime('19:00:00') and strtotime($time) < strtotime('23:23:59')){
            $day = date('w',strtotime($date));
            $weekdayName = strtolower(date("l"));
		}else{
            $date1  = date( "Y-m-d", strtotime("$date -1 day" ) );
            $day = date('w', strtotime ($date1));
            $weekdayName = strtolower(date("l",strtotime($date1)));
		}

        $lichQuay = ResultController::getLichQuaySoiCau($weekdayName);

        //check thứ để kiểm tra tỉnh thành cần lấy dữ liệu

        if (Yii::$app->request->isPost) {
            $i= 0;
            $data= array();
            foreach($lichQuay['mt'] as $maTinh){
                $tinhThanh = Yii::$app->request->post('tinhThanh'.$i);
                $soVang = Yii::$app->request->post('soVang'.$i);
                $soBac = Yii::$app->request->post('soBac'.$i);
                $kepVang = Yii::$app->request->post('kepVang'.$i);
                $capDa = Yii::$app->request->post('capDa'.$i);
                $dau = Yii::$app->request->post('soDau'.$i);
                $duoi = Yii::$app->request->post('soDuoi'.$i);
                $danDB = Yii::$app->request->post('danDB'.$i);
                $bachThu = Yii::$app->request->post('bachThu'.$i);
                
                // tạo ra 1 mảng dữ liệu
                $data[$i]["name"] = $tinhThanh;
                $data[$i]["matinh"] = $maTinh[0];
                $data[$i]["soicau"][0] ="Số Vàng";
                $data[$i]["soicau"][1] = $soVang;
                $data[$i]["soicau"][2] ="Số Bạc";
                $data[$i]["soicau"][3] = $soBac;
                $data[$i]["soicau"][4] ="Kép Vàng";
                $data[$i]["soicau"][5] = $kepVang;
                $data[$i]["soicau"][6] ="Cặp Đá";
                $data[$i]["soicau"][7] = $capDa;
                $data[$i]["soicau"][8] = "Số Đẹp Từ Chuyên Gia ";
                $data[$i]["soicau"][10] ="Sớ Đầu";
                $data[$i]["soicau"][11] = $dau;
                $data[$i]["soicau"][12] ="Sớ Đuôi";
                $data[$i]["soicau"][13] = $duoi;
                $data[$i]["soicau"][14] ="Dàn Đặc Biệt";
                $data[$i]["soicau"][15] = $danDB;
                $data[$i]["soicau"][16] ="Bạch Thủ";
                $data[$i]["soicau"][17] = $bachThu;
                $data[$i]["soicau"][18] = "Phân Tích Đặc Biệt ";

                $content["data"]= $data;
                $content["date"]= $date;
                // check tồn tại theo ngày
                $check = SoiCau::find()->where(['date'=>$date])->andWhere(['province'=>$maTinh[0]])->count();
                if($check > 0){
                    $model = SoiCau::find()->where(['date'=>$date])->andWhere(['province'=>$maTinh[0]])->one();
                    $model->province = $maTinh[0];
                    $model->date = $date;
                    $model->data = Json::encode($content,true);
                    $model->create_date = date('Y-m-d H:i:s');
                    if($model->save()){
                        $productjson = Json::encode($content,true);
                        $jsonfile = Yii::getAlias('@webroot/json/soi-cau-mien-trung.json');
                        $fp = fopen($jsonfile, 'w+');
                        fwrite($fp, $productjson);
                        fclose($fp);
                    }
            ...
            $path = Yii::getAlias('@webroot/json/soi-cau-mien-trung.json');
            $data = file_get_contents($path); 
            $data = json_decode($data,true);
            $data = $data["data"];
        ...
        return $this->render('soi-cau-mien-trung',[
        	'mt'=>$lichQuay['mt'],
            'mn'=>$lichQuay['mn'],
        	'mientrungName'=>$lichQuay['mientrungName'],
        	'miennamName'=>$lichQuay['miennamName'],
            'data'=>$data,
    	]);
    }
    public function  actionSoiCauMienNam(){
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $time = date('H:i:s');
        $date = date('Y-m-d');
        // echo $time;
        ...
        if (Yii::$app->request->isPost) {
            ...
            foreach($lichQuay['mn'] as $maTinh){
                ...
                if($check > 0){
                    ...
                    if($model->save()){
                        ...
                        $productjson = Json::encode($content,true);
                        $jsonfile = Yii::getAlias('@webroot/json/soi-cau-mien-nam.json');
                        $fp = fopen($jsonfile, 'w+');
                        fwrite($fp, $productjson);
                        fclose($fp);
                    }
            ...
            $path = Yii::getAlias('@webroot/json/soi-cau-mien-nam.json');
            $data = file_get_contents($path); 
            $data = json_decode($data,true);
             $data = $data["data"];
        ...
        return $this->render('soi-cau-mien-nam',[
        	'mt'=>$lichQuay['mt'],
            'mn'=>$lichQuay['mn'],
            'data'=>$data,
    	]);
    }
    public function actionJsonPower655(){
    	$jsonfile = Yii::getAlias('@webroot/json/power655.json');
    ...
    public function actionJsonMax4d(){
    	$jsonfile = Yii::getAlias('@webroot/json/max4d.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonMega645(){
    	$jsonfile = Yii::getAlias('@webroot/json/mega645.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonXoSoMienBac(){
    	$jsonfile = Yii::getAlias('@webroot/json/xo-so-mien-bac.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonXoSoMienTrung(){
    	$jsonfile = Yii::getAlias('@webroot/json/xo-so-mien-trung.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonXoSoMienNam(){
    	$jsonfile = Yii::getAlias('@webroot/json/xo-so-mien-nam.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonSoiCauMienNam(){
    	$jsonfile = Yii::getAlias('@webroot/json/soi-cau-mien-nam.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonSoiCauMienBac(){
    	$jsonfile = Yii::getAlias('@webroot/json/xo-so-mien-bac.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
    ...
    public function actionJsonXoSoMienNamTrung(){
    	$jsonfile = Yii::getAlias('@webroot/json/xo-so-mien-trung.json');
    	$jsondata = file_get_contents($jsonfile);
    	$obj = json_decode($jsondata, true);
    	return ([
